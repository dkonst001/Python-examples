...
# globals and constants for user interface
...
SHIP_RADIUS = 35
SHIP_THRUST_CENTER = [int(SHIP_SIZE[0] * 1.5), SHIP_SIZE[1] / 2]
ROCK_SIZE = [90, 90]
...
def group_collide(sprite_group, other):
    ...
    for a_sprite in set(sprite_group):
        if a_sprite.collide(other):
            ...
            #explosions_group.add(Sprite(a_sprite.get_pos(), [0,0], 0, 0, explosion_image, explosion_info, explosion_sound))
            sprite_group.discard(a_sprite)
            collision = True
    return collision
...
def draw(canvas):
    ...
    my_ship.draw(canvas)
    my_ship.update()
    # check collisions only 8 times per secound to imrove performance
    if time % 12 == 0:
...
nebula_image = load_image("nebula_blue.f2014.png")
...
ship_image = load_image("double_ship.png")
...
explosion_image = load_image("explosion_alpha.png")

# sound assets purchased from sounddogs.com, please do not redistribute
# .ogg versions of sounds are also available, just replace .mp3 by .ogg
pygame.mixer.init()
soundtrack = load_sound("soundtrack.mp3")
missile_sound = load_sound("missile.mp3")
#missile_sound.set_volume(.5)
ship_thrust_sound = load_sound("thrust.mp3")
